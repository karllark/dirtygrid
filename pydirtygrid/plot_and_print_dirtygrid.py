...
def plot_and_print_dirtygrid(grain, geom, sf_type, metal, age, sfr, tau, 
			     inp='indices', 
			     datafile='data/dirtygrid_interpol6.h5'):
    """
    Plots the SED for the given parameters or indices of the DIRTYGrid, 
    ...
    if inp == 'indices':
        ...
        ind_tau = sfr
  # Output SED
        sed = dirty_cube[ind_grain, ind_geom, ind_sf_type, ind_metal, 
                         ind_age, ind_sfr, ind_tau,:]
    else: # If it's values: get the closer one if the DIRTYGrid space
        # grain
        ...
        if sf_type == 'burst':
            ...
            ind_sf_type = 1
        # metal
        fd_metal = np.where((min(np.abs(param_values['metal']-metal)) 
                             == np.abs(param_values['metal']-metal)))
        ind_metal = int(fd_metal[0])
        # age
        fd_age = np.where((min(np.abs(param_values['age']-age)) 
                           == np.abs(param_values['age']-age)))
        ind_age = int(fd_age[0])
        # sfr
        fd_sfr = np.where((min(np.abs(param_values['sfr']-sfr)) 
                           == np.abs(param_values['sfr']-sfr)))
        ind_sfr = int(fd_sfr[0])
        # tau
        fd_tau = np.where((min(np.abs(param_values['tau']-tau)) 
                           == np.abs(param_values['tau']-tau)))
    ...
    print('Metallicity value in the DIRTYGrid: '
          + str(param_values['metal'][ind_metal]))
