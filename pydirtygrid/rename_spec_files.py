    ...
    for cmdir in mdirs:
        ...
        for csdir in psdirs:
            ...
            for cfile in pfiles:
                ...
                ffactors = [s for s in header["COMMENT"] if "filling_factor=" in s]
                ffactor = (ffactors[0].split("="))[-1]
                if "0.15" in ffactors[0]:
                    lgeom = "clumpy"
                else:
                    lgeom = "homo"

                newname = newname.replace(f"_{ggeom}", "")
                newname = f"{ggeom}_{lgeom}_{gtype}_{newname}"

                print(newname, cfile)

                newpath = f"{outpath}/{ggeom}/{lgeom}/{gtype}"
                print(newpath)
